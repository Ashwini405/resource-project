...
export function UploadFiles() {
  ...
  const [headers, setHeaders] = useState({ file1: [], file2: [], all: [] });
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      setExtractedData({ data1, data2 });

      const h1 = Object.keys(data1[0] || {});
      const h2 = Object.keys(data2[0] || {});
      const allHeaders = Array.from(new Set([...h1, ...h2]));

      setHeaders({ file1: h1, file2: h2, all: allHeaders });
  ...
  const UploadZone = ({ id, title, description }) => (
    <div className="relative group rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500 transition-colors">
      ...
      <div className="p-6 text-center space-y-4">
        <div className={`w-12 h-12 mx-auto rounded-full flex items-center justify-center ${files[id] ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400" : "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400"
          }`}>
      ...
      {step === 1 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <UploadZone id="file1" title="1. Base File (Master List)" description="Drag & drop .xlsx file here" />
            <UploadZone id="file2" title="2. Additional Data" description="Drag & drop .xlsx file here" />
          </div>
